...
while True:
    ...
    dilated = cv2.dilate(edgesWhite, kernel, iterations=1)
    closed = cv2.morphologyEx(edgesWhite, cv2.MORPH_CLOSE, kernel)
    ...
    cv2.imshow("Keypoints w/ normal edges", keypoints_img)
    cv2.imshow("Closed Edges (Dilation w/ some erosion)", closed)
